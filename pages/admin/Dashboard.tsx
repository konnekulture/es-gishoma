  ...
  return (
    <div className="animate-in fade-in duration-500">
      <div className="mb-10 flex flex-col md:flex-row md:items-center justify-between gap-4">
        ...
        <div className="flex items-center space-x-3 bg-white p-2 rounded-2xl shadow-sm border border-slate-100">
          ...
          <div className="pr-4">
            ...
            <div className="text-sm font-bold text-slate-700">All Nodes Operational</div>
          </div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8 mb-12">
        {cards.map((card, i) => (
          <div key={i} className="bg-white p-8 rounded-[2rem] shadow-sm border border-slate-100 relative overflow-hidden group hover:shadow-xl hover:border-indigo-100 transition-all duration-300">
            ...
            <div className={`w-14 h-14 ${card.color} rounded-2xl flex items-center justify-center text-white mb-6 shadow-lg shadow-indigo-100`}>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 mb-12">
        <div className="lg:col-span-2 bg-white p-10 rounded-[2.5rem] shadow-sm border border-slate-100 min-w-0">
          <div className="flex items-center justify-between mb-10">
            <div>
              <h3 className="text-2xl font-bold text-slate-900">Traffic Distribution</h3>
              <p className="text-sm text-slate-500 mt-1">Aggregated page views across public endpoints</p>
            </div>
          </div>
        ...
        <div className="bg-slate-900 p-10 rounded-[2.5rem] shadow-sm text-white relative overflow-hidden">
            ...
            <div className="space-y-4 flex-1">
              {[
                ...
                <div key={i} className="flex items-center justify-between p-4 bg-white/5 rounded-2xl border border-white/10">
                  <div>
                    <div className="font-bold text-slate-200 text-sm">{t.task}</div>
                    <div className="text-[10px] text-slate-500 font-black uppercase mt-1 tracking-widest">{t.status}</div>
                  </div>
                  <div className="text-indigo-400 font-black text-xs">{t.val}</div>
                </div>
              ))}
            </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
        <div className="bg-white p-10 rounded-[2.5rem] shadow-sm border border-slate-100 min-w-0">
           <div className="flex items-center space-x-3 mb-8">
              <MousePointer2 className="w-5 h-5 text-indigo-600" />
              <h3 className="text-xl font-bold text-slate-900">Page Popularity Index</h3>
            </div>
              ...
              {[
                ...
                <div key={i} className="flex items-center justify-between group">
                  <div className="flex items-center space-x-4">
                    ...
                    <div>
                      <div className="text-sm font-bold text-slate-700">{act.page}</div>
                    ...
                    <div key={res.id} className="p-6 bg-slate-50 rounded-3xl border border-slate-100 flex items-start gap-5">
                      ...
                      <div className="flex-1">
                        <div className="flex items-center justify-between mb-1">
                          <h4 className="font-bold text-slate-900">{res.label}</h4>
                          <span className={`text-sm font-black ${res.status === 'ok' ? 'text-emerald-600' : 'text-red-600'}`}>{res.value}</span>
                        </div>
