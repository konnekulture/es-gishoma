  ...
  return (
    <div className="animate-in fade-in duration-500 max-w-full overflow-hidden">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 mb-8 sm:mb-12">
        <div>
          <h1 className="text-3xl sm:text-4xl font-black text-slate-900 mb-2">Curriculum Materials</h1>
          <p className="text-slate-500 font-medium text-sm sm:text-base">Manage course books and educational PDF resources.</p>
        </div>
        <button 
        ...
        >
          ...
          <span className="text-sm sm:text-base">Add New Material</span>
        </button>
      </div>
            ...
            <table className="w-full text-left min-w-[700px]">
              <thead>
                <tr className="bg-slate-50">
                  <th className="px-6 sm:px-8 py-4 sm:py-5 text-[10px] font-black uppercase tracking-widest text-slate-500">Material</th>
                  <th className="px-6 sm:px-8 py-4 sm:py-5 text-[10px] font-black uppercase tracking-widest text-slate-500">Category</th>
                  <th className="px-6 sm:px-8 py-4 sm:py-5 text-[10px] font-black uppercase tracking-widest text-slate-500">File Info</th>
                  ...
                  <tr key={book.id} className="hover:bg-slate-50 transition-colors">
                    <td className="px-6 sm:px-8 py-5 sm:py-6">
                      <div className="flex items-center space-x-4">
                        ...
                        <div className="min-w-0">
                          <div className="font-bold text-slate-900 truncate max-w-[200px]">{book.title}</div>
                    ...
                    <td className="px-6 sm:px-8 py-5 sm:py-6 text-right">
                      <div className="flex items-center justify-end space-x-2">
                        <button onClick={() => handleOpenModal(book)} className="p-2 text-slate-400 hover:text-indigo-600 hover:bg-indigo-50 rounded-lg transition-all" aria-label="Edit">
                          <Edit2 className="w-5 h-5" />
                        </button>
                        <button onClick={() => confirmDelete(book.id)} className="p-2 text-slate-400 hover:text-red-500 hover:bg-red-50 rounded-lg transition-all" aria-label="Delete">
        ...
        <div className="fixed inset-0 z-[110] bg-slate-950/40 backdrop-blur-sm flex items-center justify-center p-4 sm:p-6 animate-in fade-in duration-300">
          <div className="bg-white w-full max-w-md rounded-[2rem] shadow-2xl overflow-hidden animate-in zoom-in-95 duration-300">
            <div className="p-8 sm:p-10 text-center">
              ...
              <h3 className="text-xl sm:text-2xl font-black text-slate-900 mb-2">Delete Resource?</h3>
              <p className="text-sm sm:text-base text-slate-500 font-medium leading-relaxed">This book will be removed from the public curriculum library. This action cannot be undone.</p>
        ...
        <div className="fixed inset-0 z-[100] bg-slate-900/60 backdrop-blur-sm flex items-center justify-center p-4 sm:p-6 animate-in fade-in duration-300">
          <div className="bg-white w-full max-w-2xl rounded-[2rem] sm:rounded-[2.5rem] shadow-2xl overflow-hidden animate-in zoom-in-95 duration-300 max-h-[90vh] flex flex-col">
            <div className="p-6 sm:p-10 border-b border-slate-100 flex items-center justify-between shrink-0">
              <h2 className="text-2xl sm:text-3xl font-black text-slate-900">{editingBook ? 'Edit' : 'Add'} Material</h2>
            ...
            <form onSubmit={handleSave} className="p-6 sm:p-10 overflow-y-auto custom-scrollbar space-y-6 sm:space-y-8">
              ...
              <div className="space-y-2">
                ...
                <select 
                  ...
                  className="w-full px-5 py-3 sm:px-6 sm:py-4 bg-slate-50 border border-slate-100 rounded-xl sm:rounded-2xl outline-none focus:ring-4 focus:ring-indigo-50 font-medium appearance-none text-sm sm:text-base"
                >
              ...
              <div className="space-y-2">
                ...
                <div className="w-full p-6 sm:p-8 rounded-xl sm:rounded-2xl bg-slate-50 border-2 border-dashed border-slate-200 flex flex-col items-center justify-center relative group min-h-[140px]">
                  ...
                  <p className="text-xs sm:text-sm font-bold text-slate-700 text-center px-4 line-clamp-2">{fileName || 'Click to select PDF book'}</p>
